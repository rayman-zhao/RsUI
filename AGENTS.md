...
- **`Page`** — `public protocol Page: AnyObject` ([`Sources/RsUI/Models/Page.swift`](./Sources/RsUI/Models/Page.swift)). Meta info `var url: URL` + `var title: String`; UI `var header: Any?` (default `nil`) and `var content: UIElement`. The lifecycle hook `func windowContextDidChange(to: WindowContext)` (default no-op) is called when the page is moved to another window or the host window enters/exits fullscreen (Page impls that cache a `WindowContext` should rebind it here, and flip fullscreen UI affordances). The public free function `startObserving(emitting:onChanged:)` ([`Support/Observation.swift`](./Sources/RsUI/Support/Observation.swift)) drives the UI off an `@Observable` state via the `Observations` async sequence (toolchain `Observation` module); it returns a cancellable `Task<Void, Never>`, and `onChanged` returns `Bool` — return `false` to stop observing from inside the callback (`true` = keep). There is no weak-owner lifecycle: closures strongly capture what they drive (see the Observation driver bullet below), so the caller owns termination — in-band `false`, or cancel the Task on page `deinit` / content rebuild.
...
Sources/RsUI/
  ...
  Controls/
    NavigationViewItem+Extensions.swift — `static build(iconGlyph:label:url:)` factory (the url is stored in `tag` as `HString`)
  ...
  Support/
    ...
    Observation.swift                   — `startObserving(emitting:onChanged:)`: the single observation entry point (`Observations` async sequence + MainActor callback; no owner — caller owns termination: in-band `false` or Task `cancel()`)
    ...
    Window+Extensions.swift             — `useMicaBackdrop()`, `useRestoration(_:)`; defines private `WindowPosition` PreferenceValue
  Models/
    ...
    Page.swift                          — `public protocol Page: AnyObject` (url/title/header/content + `windowContextDidChange(to:)`)
...
- **Event handling**: `addHandler { [weak self] _, args in ... }` with weak capture lists — weak for the Swift owner only; projected controls must be captured strongly, see [WinRT Wrapper Lifetime and Closure Captures](#winrt-wrapper-lifetime-and-closure-captures). Inside the handler, prefer calling ViewModel / model methods over mutating controls directly.
...
- **Observation driver**: UI reacts to `@Observable` state through the `Observations` async-sequence helper (from the toolchain's `Observation` module), surfaced via the single public free function `startObserving(emitting:onChanged:)` in [`Support/Observation.swift`](./Sources/RsUI/Support/Observation.swift), which returns a cancellable `Task<Void, Never>`. There is **no weak-owner lifecycle**: closures strongly capture the state source and the controls they drive — pure projection wrappers MUST be strongly captured (`[weak]` on them is `nil` by the first async callback; weak owners proved unreliable and forced the old `ProgressBarEx`/`ProgressRingEx` workaround, since removed), while Swift subclasses of projected controls may be weakly captured once attached to the visual tree — see [WinRT Wrapper Lifetime and Closure Captures](#winrt-wrapper-lifetime-and-closure-captures) — so every call site owns termination: stop in-band by returning `false` from `onChanged` (`true` = keep observing), or cancel the returned `Task` when the UI it drives goes away (page `deinit`, content rebuild, window `closed`). Prefer this flow: emit the relevant ViewModel state, run the update on `MainActor`, and mutate UI only inside the `onChanged` callback; event handlers call ViewModel methods instead of mutating controls directly (see `AppearancePage` for the full pattern). (The concrete `appearanceChanged` / `fullscreenChanged` shells — `AppearanceWindow` and `NavigationViewWindow` — additionally expose a plain `EventHandler`/`EventWithArgumentHandler` for surface-level one-shot fan-out that is not tied to a long-lived ViewModel; `addHandler` returns an `EventHandlerToken` and `removeHandler(_:)` unregisters.)
...
### WinRT Wrapper Lifetime and Closure Captures
- A "WinUI control" comes in two kinds with **different lifetime semantics** — which kind a closure captures decides whether `[weak]` can work:
  - **Pure projection wrappers** — types constructed directly from the projection (`ProgressRing()`, `TextBlock()`, …) — are thin Swift classes (`open class ProgressRing : WinUI.Control`, bottoming out in `WindowsFoundation.IUnknown` holding a single COM reference). Retaining is one-way: Swift wrapper → COM ref → native object. Appending the control to the visual tree AddRefs only the *native* object; nothing native ever points back at the wrapper, so the wrapper survives on Swift ARC alone. Once the creating scope returns and no Swift property holds it, ARC deallocates the wrapper and any `[weak]` capture reads `nil` — typically **before the first async callback fires** (`startObserving`'s `onChanged`, `addHandler`, `Task`). There is also no identity table for pure wrappers: fetching the same native object again (e.g. `findName`) returns a brand-new wrapper instance, so a strong reference held elsewhere to "the same control" cannot keep *this* wrapper alive. This is the mechanism behind the historical `ProgressBarEx`/`ProgressRingEx` workaround.
  - **Swift subclasses of projected classes** — `class X: WinUI.NavigationViewItem`, e.g. RsUI's `NavigationViewItemWithAction`, `SettingsCard`, `PageTransitionHost`, `RangeSlider` — are created through WinRT composition (aggregation). The projection installs a `WinRTClassWeakReference` bridge (swift-windowsfoundation `AGGREGATION.swift`) that forwards every native `AddRef`/`Release` on the composed object into `retain()`/`release()` of the Swift instance. Membership in the visual tree therefore keeps the Swift instance alive; it is released only when the element is removed from the tree or the window is torn down. On these, `[weak]` IS a meaningful lifetime signal: `guard let item else { return false }` correctly stops observation when the UI goes away (the pattern used by downstream Ruslan's `TriageModule`).
- **Rules**:
  - Closures that execute later than the current scope must capture **pure projection wrappers strongly**. `[weak control]` on a pure wrapper is always a bug: the first async callback sees `nil` and the UI update silently never happens (no crash — double-check any "why is my observed control nil" report).
  - On **Swift-subclassed controls** already attached to the tree, weak capture is valid; strong capture remains the safer default — the pre-attach window (constructed but not yet appended) is covered only by local strong references, which is why `NavigationViewItemWithAction`'s internal event closures capture `self` strongly.
  - A strong capture means the closure owns the control's survival (wrapper → COM ref → native object): give the observation an endpoint — in-band `return false`, or cancel the returned `Task` on UI rebuild / page `deinit` / window `closed`.
  - `[weak self]` on your own `@Observable` modules / ViewModels is unaffected — those are genuinely retained by Swift-side strong graphs (e.g. `App.context.modules`).
- **How to tell them apart**: look at the declaration — `class X: SomeWinUIType` in your/framework code is a subclass; anything constructed directly as `WinUI.Xxx()` is a pure wrapper.
...
- Taskbar "New Window" is the JumpList entry registered in `App.onLaunched` via `JumpList.register(arguments: "--new-window", …)`. A launch with `--new-window` reaches the primary instance through the single-instance redirect path: the second instance's activation args are redirected, the primary's `activated` handler (`App.onActivated(_:)`) fires and activates a fresh `MainWindow`. `onLaunched` itself does not inspect `--new-window`.
